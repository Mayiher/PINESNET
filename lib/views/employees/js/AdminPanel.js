...
async function handleSave() {
  ...
  console.log('→ handleSave', { method: 'POST', url, data });
  ...
  try {
    ...
    if (!res.ok) {
      let err;
      try { err = JSON.parse(payload); }
      catch { err = { error: payload }; }
...
async function handleDelete(section, id) {
  ...
  console.log('→ handleDelete', { method: 'DELETE', url });

  try {
    const res = await fetch(url, { method: 'DELETE' });
    console.log('← DELETE status:', res.status);
    const payload = await res.text();
    console.log('← DELETE body:', payload);
